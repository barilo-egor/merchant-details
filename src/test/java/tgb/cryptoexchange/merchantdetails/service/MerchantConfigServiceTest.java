...
class MerchantConfigServiceTest {
    ...
    void findAllShouldReturnMatchConfigIfOneOfOneMatchToOneMethod() {
        ...
        methods.add(DetailsRequest.MerchantMethod.builder().merchant(Merchant.ALFA_TEAM).methods(Collections.singletonList("SBP")).build());
    ...
    void findAllShouldReturnNoConfigIfNoOneOfFewMatch(Merchant merchant, String method) {
        ...
        when(merchantConfigRepository.findAllByIsOnOrderByMerchantOrder(any())).thenReturn(merchantConfigs);
        List<DetailsRequest.MerchantMethod> methods = new ArrayList<>();
        methods.add(DetailsRequest.MerchantMethod.builder().merchant(merchant).methods(Collections.singletonList(method)).build());
        List<MerchantConfig> actual = merchantConfigService.findAllByMethodsAndAmount(methods, 5500);
        assertAll(
                () -> assertEquals(0, actual.size())
        );
    ...
    void findAllShouldReturnMatchConfigIfOneOfFewMatchToOneMethod(String method) {
        ...
        when(merchantConfigRepository.findAllByIsOnOrderByMerchantOrder(any())).thenReturn(merchantConfigs);
        List<DetailsRequest.MerchantMethod> methods = new ArrayList<>();
        methods.add(DetailsRequest.MerchantMethod.builder().merchant(Merchant.BIT_ZONE).methods(Collections.singletonList(method)).build());
    ...
    void findAllShouldReturnMatchConfigIfFewMatchToFewMethods(String method) {
        ...
        when(merchantConfigRepository.findAllByIsOnOrderByMerchantOrder(any())).thenReturn(merchantConfigs);
        List<DetailsRequest.MerchantMethod> methods = new ArrayList<>();
        methods.add(DetailsRequest.MerchantMethod.builder().merchant(Merchant.ALFA_TEAM).methods(Collections.singletonList(method)).build());
        methods.add(DetailsRequest.MerchantMethod.builder().merchant(Merchant.BIT_ZONE).methods(Collections.singletonList(method)).build());
        methods.add(DetailsRequest.MerchantMethod.builder().merchant(Merchant.SETTLE_X).methods(Collections.singletonList(method)).build());
